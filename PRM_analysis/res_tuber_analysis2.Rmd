...
*hmmmm so, it seems that by not including Garnett Chili as part of Chiletanum has a effect on the predtub*
...
Plotting abline + residuals of S vs residuals of live soil..
```{r}
...
############################################################################
Let's do the above method again but this time by Variety(In progress)

```{r}
library(tidyverse)
library(broom)

max = read.csv("tuber_analysis_2.csv")

#regress tuber mass vs. "sunlight" for each clade and create detrended dataset
bertdata = subset(max, agerank2 == 1)
bert = lm(tuber3~sunlight, data=bertdata) 
#Makes a linear regression between tuber yield and sunlight 
fakesunlight = rep(3, length(bert$res))
#Make a new vector for "fake" sunlight where the value is 3 that is the same lenght as the # of residuals for bert. 
...
#So to scale this up and not do it 17(x) times we will use dyplyr: 

tuber_avg = max %>%
  group_by(variety2,Nutrient) %>%
  do(fitted_tuber = lm(tuber3 ~ sunlight, data = .))

fakesunlight = rep(3, length(1))
newdata = data.frame(sunlight=fakesunlight)
 
predtub = predict(tuber_avg$fitted_tuber[[1]], newdata) + tuber_avg$fitted_tuber[[1]]$res

#why does this make a vector of 12 predicted values, it looks like the residual objective contains 12 residuals...should we average them? Is it okay to do that?
```
...
```{r}
genoavg1 = aggregate(cbind(predtub,variety2) ~ Nutrient + soil + variety2, data=max2, FUN=mean)


genoavgH_1 = subset(genoavg, soil == "Hancock")
genoavgV_1 = subset(genoavg, soil == "Virgin")
genoavgS = subset(genoavg, soil == "Sterile")

genoavgH_1 = cbind(genoavgH, genoavgS$predtub)
genoavgV_1 = cbind(genoavgV, genoavgS$predtub)

genoavg2_1 = rbind(genoavgH, genoavgV)
colnames(genoavg2_1)[6]<-'predtubS'

genoavg2_1 = genoavg2_1[order(genoavg2_1$Nutrient),]

genoavg2_1 = cbind(genoavg2_1, as.factor(genoavg2_1$agerank2))
colnames(genoavg2_1)[7] <- 'agerankcat'

#high nutrients:

a = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="High" & soil == "Hancock"))

b = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="High" & soil == "Virgin"))

c = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="Low" & soil == "Hancock"))

d = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="Low" & soil == "Virgin"))
...
```{r}
a = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="High"))

b = lm(predtub~predtubS, data = subset(genoavg2_1, Nutrient=="Low"))
```
...
genoavg2_1 = cbind(genoavg2_1, res)

e = lm(predtub~predtubS, data = genoavg2_1)
res2_1 = e$res
genoavg2 = cbind(genoavg2_1, res2_1)

genoavg2_LV35 = subset(genoavg2_1, Nutrient=="Low" & soil=="Virgin"&(agerank2 == 3|agerank2 == 5))

genoavg4 = subset(genoavg2_1, agerank2 != 4)
```


```{r}
#high nutrients
a = lm(predtub~predtubS, data = subset(genoavg3, Nutrient=="High"))

#low nutrients
b = lm(predtub~predtubS, data = subset(genoavg3, Nutrient=="Low"))
```
